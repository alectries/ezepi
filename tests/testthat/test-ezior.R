...
test_that("ezior generates correctly",{
  data(mtcars)
  ior <- oddsratio(8, 3, 3, 4) %>% unlist() %>% unname()
  expect_equal(
    ezior(mtcars, cyl, am, index_exp = 4, ref_exp = 6),
    tibble(
      item = c("Odds Ratio", "LCI", "UCI", "p-value"),
      result = c(
        ior %>% nth(4) %>% as.numeric(),
        ior %>% nth(2) %>% as.numeric(),
        ior %>% nth(3) %>% as.numeric(),
        ior %>% nth(1) %>% as.numeric()
      )
    )
  )
})
...
test_that("ezior fails without data",{
  expect_error(
    ezior(nodata, cyl, am, index_exp = 4, ref_exp = 6)
  )
})

# test that the function runs incorrectly without exposure var
test_that("ezior fails without exp",{
  data(mtcars)
  expect_error(
    ezior(mtcars),
...
test_that("ezior fails without out",{
  data(mtcars)
  expect_error(
    ezior(mtcars, cyl),
...
test_that("ezior fails with bad exp input class",{
  data(mtcars)
  expect_error(
    ezior(mtcars, cyl, am, index_exp = "4", ref_exp = "6"),
...
test_that("ezior fails with bad exp input values",{
  data(mtcars)
  expect_error(
    ezior(mtcars, cyl, am, index_exp = 3, ref_exp = 5),
    regexp = "ezepi: Error: index/referent value does not exist in exposure_var"
  )
})

# test that the function runs incorrectly with wrong class input for our
test_that("ezior fails with bad out input class",{
  data(mtcars)
  expect_error(
    ezior(mtcars, cyl, am, index_exp = 4, ref_exp = 6, index_out = "1", ref_out = "0"),
...
test_that("ezior fails with bad out input values",{
  data(mtcars)
  expect_error(
    ezior(mtcars, cyl, am, index_exp = 4, ref_exp = 6, index_out = 3, ref_out = 5),
