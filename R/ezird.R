...
ezird <- function(x,
  ...
  ezird.res <- as_tibble(data.frame(
    ...
    result = c(
      ezird.fmsb %>% unlist() %>% unname() %>% nth(4) %>% as.numeric(),
      ezird.fmsb %>% unlist() %>% unname() %>% nth(2) %>% as.numeric(),
      ezird.fmsb %>% unlist() %>% unname() %>% nth(3) %>% as.numeric(),
      ezird.fmsb %>% unlist() %>% unname() %>% nth(1) %>% as.numeric()
    )
  ))
